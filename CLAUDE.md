...
- **v29 fix:** Added `generateImage` to the batchExecute exclusion list. Observed in prod (Langfuse): Haiku wrapped generateImage inside batchExecute, causing "1/2 operations failed" because batchExecute cannot await the async Workers AI call. generateImage now listed with explicit reason: "(async image generation - batchExecute cannot await it)".
...
- **Quality signal feature:** Deployed in v26 (commit 306f76e), gated by `QUALITY_SIGNAL_ENABLED="true"` env var (off by default). Per-turn Haiku judge scoring: yesAnd, characterConsistency, sceneAdvancement, toolAppropriateness (0-3 each). Fires via ctx.waitUntil after reactive persona. Safe to enable in prod: logs only numeric scores + dimension names (rawResponse with reasoning field is never logged). Scores attached to Langfuse `quality-signal` traces grouped by boardId sessionId.
...
- **Langfuse gaps (as of v29):** (1) `totalCost` is $0.00 on all traces - tracing middleware doesn't pass model pricing to Langfuse. Fixed in recent release (tracing-middleware.ts now computes `inputCost`/`outputCost` from MODEL_PRICING_PER_MTOK). (2) Quality signal scores are now attached to Langfuse `quality-signal` traces with `sessionId=boardId` - grouped with chat traces in the same session view. (3) Quality signal traces are not directly linked to the specific chat trace that triggered scoring (tracing middleware creates traces internally without exposing traceId).
