...
- **Browser tool:** Always say `playwright-cli` explicitly. Agents default to `agent-browser` when unspecified (per global CLAUDE.md fallback rule), but `agent-browser` lacks `-s` multi-session support. playwright-cli is installed in this repo - name it every time.
